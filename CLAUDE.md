...
- **The `ApiVersions` request itself is a bootstrapping problem.** You don't know the broker's supported range until you've asked, so send at our max and treat error code 35 `UNSUPPORTED_VERSION` as "retry at v0". Do not treat it as a fatal handshake failure — and do not adopt the table in the error response as the broker's api surface: per KIP-511 it names **only the `ApiVersions` key itself** (the retry instruction). Both mistakes were made and caught against a Kafka 3.7 broker; any 3.9+/4.x broker accepts our max, so the fallback path only ever runs against older brokers and `kaas`-like implementations.
