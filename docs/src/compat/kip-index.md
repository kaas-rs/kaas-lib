...
|---|---|---|
| KIP-98 | Exactly-once: idempotent producer + transactions | ⚠️ read side only |
| KIP-227 | Incremental fetch sessions | ❌ deliberately session-less |
| KIP-345 | Static consumer membership | ❌ no membership |
| KIP-368 | SASL re-authentication | ✅ |
| KIP-405 | Tiered storage | ⚠️ `-4` sentinel surfaced |
| KIP-447 | Exactly-once v2 | ⚠️ read side only |
| KIP-480 | Sticky partitioner | ❌ no producer |
...
| KIP-848 | The next-generation consumer rebalance protocol | ⚠️ describe only |
...
### KIP-848 — the new consumer group protocol ⚠️
...
`ConsumerGroupDescribe` is implemented, so a UI can render these groups
completely — epochs, assignor, per-member assignment. **Membership is not.**
Joining one requires `ConsumerGroupHeartbeat`, which is on the
[roadmap](../guide/roadmap.md) and is where the protocol should be adopted
first, because server-side assignment removes the byte-compatibility problem
that makes the classic protocol hard.
...
Deliberately not used. `crates/kafka-read/src/fetch.rs` pins
`session_id = 0, session_epoch = -1` — Java's `FetchMetadata.LEGACY`
sentinel.

A UI's scans are one-shot, and an incremental session would make each scan
depend on the last for no benefit. It is exactly the wrong choice for a
steady-state consumer, which is why adopting it is a prerequisite for group
membership on the [roadmap](../guide/roadmap.md).

### KIP-98 / KIP-447 — exactly-once ⚠️

The read half only. `Visibility::CommittedOnly` sets `read_committed`, and
the client filters aborted records using the `AbortedTransactions` list the
broker returns — the broker does not filter for you. `DescribeTransactions`,
`ListTransactions` and `DescribeProducers` let a UI inspect transaction
state.

The write half — `InitProducerId`, `AddPartitionsToTxn`, `EndTxn` — needs a
producer.
